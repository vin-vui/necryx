            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <ul class="ml-8 flex flex-row font-medium space-x-8 md:text-sm md:border-0 md:bg-transparent dark:bg-white-800 md:dark:bg-white-900 dark:border-gray-700">
                        <li>
                            <button id="dropdownNavbarLink" data-dropdown-toggle="dropdownNavbar" class="flex items-center justify-between rounded md:border-0 text-base"> Collection
                                <svg class="w-2.5 h-2.5 ml-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4" />
                                </svg>
                            </button>
                            <!-- Dropdown menu -->
                            <div id="dropdownNavbar" class="z-20 hidden w-44 bg-seasalt text-onyx divide-y divide-gray-100 rounded-lg shadow">
                                <ul class="mt-4 py-2 text-sm dark:text-gray-400" aria-labelledby="dropdownLargeButton">
                                    <li>
                                        <a href="{{ route('collections', 'jeu') }}" class="block px-4 py-2 font-p text-onyx">Jeux</a>
                                    </li>
                                    <li>
                                        <a href="{{ route('collections', 'concept') }}" class="block px-4 py-2 font-p text-onyx">Concepts</a>
                                    </li>
                                    <li>
                                        <a href="{{ route('collections', 'art') }}" class="block px-4 py-2 font-p text-onyx">Art</a>
                                    </li>
                                </ul>
                            </div>
                        </li>
                        <li>
                            <a href="{{ route('concepts') }}" class="md:p-0 text-onyx py-2 pl-3 pr-4 text-base">Concepts & Conseils</a>
                        </li>
                        <li>
                ...
                <div class="hidden lg:flex mr-8">
                    ...
                    <div class="hidden lg:ml-4 lg:flex lg:items-center ">
                        ...
                        <div class="lg:relative ml-4 flex-shrink-0" x-data="{ open: false }">
                            ...
                            <button type="button" @click="open = !open" class="flex rounded-full hover:scale-110 duration-300 bg-seasalt text-sm focus:outline-none focus:ring-2 focus:ring-black-bean-500 focus:ring-offset-2">
                                <span class="sr-only">Ouvrir le menu</span>
                                <img class="h-11 w-11 rounded-full " src="{{ Auth::user()->profile_photo_url }}" alt="{{ Auth::user()->name }}">
                            </button>

                            <div x-show="open" x-transition.scale.origin.top x-transition.delay.80ms @click.away="open = false" class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-seasalt py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                                <!-- Active: "bg-gray-100", Not Active: "" -->
                                <a href="{{ route('profile.show') }}" class="flex  px-4 py-2 text-base font-medium text-black-bean font-p hover:underline" role="menuitem" tabindex="-1" id="user-menu-item-0">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" class="mr-3" viewBox="0 0 24 24">
                                        <g fill="none">
                                ...
                                <form method="POST" action="{{ route('logout') }}" x-data>
                                    @csrf
                                    <a href="{{ route('logout') }}" @click.prevent="$root.submit();" class="flex items-center  px-4 py-2 text-onyx font-p hover:underline">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" class="mr-3" viewBox="0 0 24 24">
                ...
                <a href="{{ route('register') }}" class="inline-flex items-center justify-center px-2.5 font-p text-base font-medium leading-6 text-white whitespace-no-wrap bg-yellow-400 hover:bg-indigo-400border rounded-md">
                    S'inscrire
                </a>
            </div> --}}
            </div>
