            ...
            <div class="mx-auto grid max-w-2xl grid-cols-1 items-start gap-x-8 gap-y-16 sm:gap-y-24 lg:mx-0 lg:max-w-none lg:grid-cols-2">
                <div class="lg:pr-4">
                    <div class="relative overflow-hidden rounded-3xl bg-gray-900 px-6 pb-9 pt-64 shadow-2xl sm:px-12 lg:max-w-lg lg:px-8 lg:pb-8 xl:px-10 xl:pb-10">
                        <img class="absolute inset-0 h-full w-full object-cover" src="{{ Storage::url($collection->image) }}" alt="">                       
                ...
                <div>
                    <div class="text-base leading-7 text-gray-700 lg:max-w-lg">
                        ...
                        <div class="max-w-xl">
                            ...
                            <p class="mt-6 font-p">{!! $collection->origin!!}</p>
                            <p class="mt-6 font-p">{!! $collection->example!!}</p>
                        </div>
                    </div>
                    <dl class="mt-10 grid grid-cols-2 gap-8 border-t border-gray-900/10 pt-10 sm:grid-cols-4">
                        @if($collection->types === 'jeu')
                        <div>
                            <dt class="text-sm font-semibold leading-6 text-gray-600">Nombre de joueurs</dt>
                            <dd class="mt-2 text-3xl font-bold leading-10 tracking-tight text-gray-900">{{ $collection->number_players }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-semibold leading-6 text-gray-600">Durée</dt>
                            <dd class="mt-2 text-3xl font-bold leading-10 tracking-tight text-gray-900">{{ $collection->duration }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-semibold leading-6 text-gray-600">Age</dt>
                            <dd class="mt-2 text-3xl font-bold leading-10 tracking-tight text-gray-900">{{ $collection->age }}</dd>
                        </div>
                        @endif
                    </dl>
                    <div class="mt-10 flex">
                        ...
                        class="text-base font-semibold leading-7 text-dark-red">Découvrez nos autres jeux <span aria-hidden="true">&rarr;</span></a>
                        @elseif($collection->types === 'concept')
                        <a href="{{ route('collections', 'concept') }}"
                        class="text-base font-semibold leading-7 text-dark-red">Découvrez nos autres concepts <span aria-hidden="true">&rarr;</span></a>
                        @else($collection->types === 'art')
                        <a href="{{ route('collections', 'art') }}"
                        class="text-base font-semibold leading-7 text-dark-red">Découvrez notre galerie d'art <span aria-hidden="true">&rarr;</span></a>   
                        @endif
                    </div>
                </div>
            </div>
